...
export const AspirationsPage = () => {
    ...
    const { user } = useAuth(); // fallback to fetch from DB

    // const { suggestions: navSuggestions, aspirationalLinkedinUrl: navUrl } = location.state || {};

    const [suggestions, setSuggestions] = useState<Array<any>>([]);
    const [aspirationalLinkedinUrl, setAspirationalLinkedinUrl] = useState<string | undefined>();
    ...
    const [selectedTargetUrl, setSelectedTargetUrl] = useState<string | undefined>();
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchSuggestions = async () => {
            if (user?.aspirationalLinkedinUrl) {
                try {
                    setLoading(true);
                    const { data } = await roadmapCalibrationApi.post('/suggest-aspirations', {
